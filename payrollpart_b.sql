...
/*
1. Display for all employees their employee number, name, job, monthly salary, their 
...
/*
Q4. Display all employees details in the following format: 
EMPLOYEE N. Smith IS A Trainer AND WORKS IN THE Training DEPARTMENT.
Order the output by employee number.
*/
...
/*
6. Display the employee name and birthdate for all employees who were born in 
February. Order the output by employee name.
*/
...
/*
7. Display the employee name, salary and commission for those employees who 
earn more commission than their monthly salary (using the GREATEST function). 
Order the output by employee name and within name by monthly take home pay 
...
/*
9. For all employees who have registered for any course, display the employee 
number, name, and the total number of registrations they have made. 
Order the output by employee number.
*/
...
/*
10. Who is the oldest employee? Show the employee number, name and their date of 
...
the average salary paid to their employees. Name the columns: NbrOfEmployees, 
MinSalary, MaxSalary, TotalSalary, AvgSalary. If the number of employees for a 
department is 0 display a blank value (a space) for MinSalary, MaxSalary, 
TotalSalary and AvgSalary. Order the output by department number.
*/

SELECT
    d.deptno,
    d.deptname,
    COUNT(e.empno)                                AS "NbrOfEmployees",
    nvl(to_char(MIN(empmsal),'9999.99'),' ')      AS "MinSalary",
    nvl(to_char(MAX(empmsal),'9999.99'),' ')      AS "MaxSalary",
    nvl(to_char(SUM(empmsal),'9999999.99'),' ')   AS "TotalSalary",
    nvl(to_char(AVG(empmsal), '9999.99'),' ')     AS "AvgSalary"
FROM
    payroll.employee      e
...
SELECT
    deptno,
    empjob,
    SUM(empmsal) as TOTAL_MNTH_SALARY
FROM
    payroll.employee
GROUP BY
    deptno,
